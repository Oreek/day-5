...
def create_gift():
    if not passwd:
        return flask.jsonify({"error": "password not setup"}), 500
    
    data = flask.request.get_json()
    name = data.get('name')
    gift = data.get('gift')
    submit_passwd = data.get('password')
    
    if submit_passwd != passwd:
        return flask.jsonify({"error": "nice try clanker"}), 401
